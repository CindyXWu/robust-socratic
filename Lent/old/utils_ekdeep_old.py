...
### Dataloaders
def get_dataloader(
    load_type='train', base_dataset='CIFAR10', spurious_type='plain', spurious_corr=1.0, spurious_intensity=1., 
    ...
    Args:
        ...
        use_spurious_by_ids: 
        randomize_img: whether to randomize image (for dominoes)
        data_dir: directory to store data (usually 'data')
        name: name of custom dataset (specify which augmentation used etc)
    """
    # define transforms
    ...
    # The first time, set download=True to download the dataset
    ...
    # define dataloader
    dataloader = DataLoader(dset, batch_size=batch_size, shuffle=(load_type=='train'), num_workers=1, drop_last=True)
    return dataloader
