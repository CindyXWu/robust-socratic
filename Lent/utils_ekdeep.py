...
class domCueDataset(Dataset):
    def __init__(self, dataset, dataset_simple, cue_proportions=[1., 1.], randomize_cues=[False, False], randomize_img=False):
        ...
        # cue information
        self.mnist_proportion, self.box_proportion, self.randomize_mnist, self.randomize_box, self.randomize_img = cue_proportions[0], cue_proportions[1], randomize_cues[0], randomize_cues[1], randomize_img
        self.mnist_cue_ids = get_cue_ids(targets=self.targets, n_classes=self.n_classes, prob=self.mnist_proportion)
        self.box_cue_ids = get_cue_ids(targets=self.targets, n_classes=self.n_classes, prob=self.box_proportion)
    ...
    def __getitem__(self, item):
        ...
        if self.mnist_proportion > 0:
            put_mnist = (np.random.uniform() < self.mnist_proportion) if self.mnist_cue_ids is None else self.mnist_cue_ids[label][item]
        else:
            put_mnist = False

        if self.box_proportion > 0:
            put_box = (np.random.uniform() < self.box_proportion) if self.box_cue_ids is None else self.box_cue_ids[label][item]
        else:
            put_box = False
