...
def jacobian_loss(scores, targets, inputs, T, alpha, batch_size, loss_fn):
    ...
    s_jac = get_approx_jacobian(scores, inputs, batch_size, aggregate_chan=False)
    t_jac = get_approx_jacobian(targets, inputs, batch_size, aggregate_chan=False)
    
    dims = [d for d in range(1, s_jac.dims)]    # Don't add batch dimension to norm calculation
...
def get_approx_jacobian(output, x, batch_size):
    ...
    grad_output.view(batch_size, -1)[:, i] = 1
    jacobian = torch.autograd.grad(output, x, grad_output, retain_graph=True)
